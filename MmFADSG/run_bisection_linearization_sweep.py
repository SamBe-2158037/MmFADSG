...
def main():
    ...
    parser.add_argument('--dataset-name', type=str, default='cosponsorIt', metavar='S', help='Desired Dataset Name')
    ...
    parser.add_argument('--alpha', type=float, default=0.05, metavar='R', help='corretion level for LSE')
   ...
   # density_0 = exact_R[1]
    ...
    for target_etha in target_etha_range:
        lam_max_ = lam_max
        lam_min_ = 0
        res = super_greedy_pp_R = super_greedy_pp_lse_qreg_SSP(
                G, G, protected_nodes, lam=0,
                mu=mu, alpha=alpha,
                num_passes=T, num_outer=num_outer
            )
        converged = False
        #S = G.subgraph(res[0])  # initial linearization point is the full graph
        for i in tqdm(range(max_iters)):
            # ---- Pick midpoint and run solver there ----
            lam_mid = (lam_max_ + lam_min_) / 2
            lam_mid_vec.append(lam_mid)

            super_greedy_pp_R = super_greedy_pp_lse_qreg_SSP(
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(f'{dataset_name}/bisection_sweep_results_{dataset_name}_lin1_tune.pdf')
    plt.show()
